...
# cmdline.execute("scrapy crawl deepsearch_stock_info_spider -a target_date=2021-06-30 -a scraping_count_goal=20".split())

today = time.localtime(time.time())
if (datetime.now() >= datetime(today.tm_year,5,18)) & (datetime.now() < datetime(today.tm_year,8, 17)):
    cmdline.execute(
        ("scrapy crawl deepsearch_stock_info_spider -a target_date="+str(today.tm_year)+"-03-31 -a scraping_count_goal=180").split())

elif (datetime.now() >= datetime(today.tm_year,8,17)) & (datetime.now() < datetime(today.tm_year,11, 16)):
    cmdline.execute(
        ("scrapy crawl deepsearch_stock_info_spider -a target_date=" + str(today.tm_year) + "-06-30 -a scraping_count_goal=180").split())

elif datetime.now() >= datetime(today.tm_year,11, 16):
    cmdline.execute(
        ("scrapy crawl deepsearch_stock_info_spider -a target_date=" + str(today.tm_year) + "-09-30 -a scraping_count_goal=180").split())
elif datetime.now() < datetime(today.tm_year,4, 1):
    cmdline.execute(
        ("scrapy crawl deepsearch_stock_info_spider -a target_date=" + str(today.tm_year-1) + "-09-30 -a scraping_count_goal=180").split())

elif (datetime.now() >= datetime(today.tm_year,4, 1)) & (datetime.now() < datetime(today.tm_year,5, 18)):
    cmdline.execute(
        ("scrapy crawl deepsearch_stock_info_spider -a target_date=" + str(today.tm_year-1) + "-12-31 -a scraping_count_goal=180").split())
